...
s   *
  ...
  inline bool rxFlag() { return (read_register(NRF_STATUS) & _BV(RX_DR)); }
  inline void resetRxFlag() { write_register(NRF_STATUS, _BV(RX_DR)); }
  inline bool rxEmpty() { return (read_register(FIFO_STATUS) & _BV(RX_EMPTY)); }
  inline void flushRx() { flush_rx(); }
  ...
  */
  ...
  uint8_t readPayload(void* buf, uint8_t len) { return read_payload(buf, len); }
private:
