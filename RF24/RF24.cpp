...
{
	// TODO: START HERE
  uint8_t newVal = enable ? 0xFF : 0x00;
  // uint8_t newVal = 0 | enable;
  // newVal |= newVal << 1;
  // newVal |= newVal << 2;
  // newVal |= newVal << 4;


  // newVal is 8 bits of enable

  write_register(EN_AA, newVal);
...
void RF24::setCRCLength(rf24_crclength_e length)
{
	uint8_t config;
	// TODO: START HERE
  config = read_register(CONFIG);
  //config= 0b10101010;
  config &= 0b11110011;
  //config= 0b10100011;
  config |= (length != RF24_CRC_DISABLED) ? 0b00001000 : 0;
  config |= (length == RF24_CRC_16) ? 0b00000100 : 0;
...
void RF24::setRetries(uint8_t delay, uint8_t count)
{
 ...
 // Write the delay and count variables to the SETUP_RETR register
  uint8_t newVal = ((delay & 0b1111) << 4) | (count & 0b1111);
  write_register(SETUP_RETR, newVal);
 //TODO: END HERE
}
